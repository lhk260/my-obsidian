...
DreamDojo 的整体流程有三阶段，图 1 画得很清楚：先 human video pretraining，再 robot post-training，最后 autoregressive distillation。

### 阶段 1：Pretraining from human videos

在 In-lab、EgoDex、DreamDojo-HV 三类人类视频上训练 foundation world model，并为所有视频引入 latent action 条件。

### 阶段 2：Post-training on target robots
...
DreamDojo 的 human data 主要来自三部分：
...
## 7. 模型基础：Cosmos-Predict2.5

DreamDojo 不是从零训练视频模型，而是基于 **Cosmos-Predict2.5** 构建。  
...
作者训练了一个 **VAE 风格的 latent action model**：

- Encoder 输入：相邻两帧 (f_t, f_{t+1})
    
- 输出：低维 latent action (\hat a_t)
    
- Decoder 输入：(\hat a_t) 和前一帧 (f_t)
...
\mathbb E_{q_\phi(\hat a|f_{t:t+1})}\log p_\theta(f_{t+1}|\hat a,f_t)  
-\beta D_{KL}(q_\phi(\hat a|f_{t:t+1})|p(\hat a))  
]

这个信息瓶颈会迫使模型压缩“从前一帧到后一帧最关键的变化”，从而学出动作相关表征。
...
### 9.1 Relative Actions

作者不直接用绝对关节位姿，而是把动作改为 **relative action**，相对每个 latent frame 起点重基准化。  
原因是相对动作分布更集中，更利于泛化。

### 9.2 Chunked Action Injection

WAN2.2 的时间压缩比是 4，因此一个 latent frame 对应 4 个像素帧。作者把连续 **4 个动作拼成一个 chunk** 对齐到对应 latent frame，避免 future action 带来的因果混淆。

### 9.3 Temporal Consistency Loss

除了原始 flow matching loss，作者又加了 temporal consistency loss：
